...
def load_baseline_sorted_indices(output_dir):
    ...
    # Extract indices in the sorted order already provided
...
if __name__ == "__main__":
    ...
    for label_noise in args.label_noises:
        ...
        ranked_indices = np.argsort(-self_influence)  # Negative for descending order
        print(np.unique(ranked_indices))

        # Load the baseline ordering for this specific label_noise
        baseline_indices = load_baseline_sorted_indices(output_dir)
        print(np.unique(baseline_indices))
        ...
        plt.plot(fractions_checked, fractions_identified, marker='o', label='Self-Influence')
        # Plot baseline line in red
        plt.plot(fractions_checked, baseline_fractions_identified, marker='o', color='red', label='Baseline Max Prob')
